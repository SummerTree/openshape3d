...
//  openshape3dUITests
//
//  Interchange chrome (plan §B14): the Export menu lists GLB/DXF (plus the
//  USDZ/AR Preview pair only where ModelIO can write USDZ — they hide
//  together), the OBJ/GLB options sheet offers the per-body toggle, and the
//  Import menu lists the DXF entry. The system file dialogs are never
//  driven — the DXF import/export paths are unit-covered in IO2FlowTests.
//
...
final class IO2UITests: XCTestCase {
    ...
    func testExportAndImportMenusListInterchangeFormats() throws {
        ...
        // Import menu lists the DXF entry next to STL.
